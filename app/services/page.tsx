...
function AffiliateLink({ code }: { code: string }) { return <div className="affiliate-link"><small>PR</small><span dangerouslySetInnerHTML={{ __html: code }} /></div>; }

function ResourceCard({ item }: { item: Service }) { return <article className="service-card compact-service-card"><span>{categories.find((cat) => cat.id === item.category)?.name}</span><h3>{item.name}</h3><p>{item.purpose}</p><ul>{item.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><div className="service-caution"><b>確認点</b><p>{item.caution}</p></div><a href={item.url} target="_blank" rel="noopener noreferrer">{item.label} <b>↗</b></a></article>; }

export default function ServicesPage() {
  const brokers = services.filter((item) => item.category === "broker");
  const resources = services.filter((item) => item.category !== "broker");
  return <main className="services-page">
    ...
    <section className="service-purpose-section"><div className="services-heading"><p className="eyebrow">CHOOSE WHAT TO COMPARE</p><h2>まず、比較するものを選ぶ</h2><p>口座や道具を先に選ぶのではなく、学んだ結果として必要になったものだけを比較します。</p></div><div className="service-entry-grid"><a href="#broker"><b>証券会社</b><span>NISA、日本株・米国株、投資信託の口座を比べる</span></a><a href="#ideco"><b>iDeCoの金融機関</b><span>制度を理解したあとに、商品・費用・支援体制を比べる</span></a><a href="#resources"><b>練習・分析・参考書</b><span>デモ取引、チャートツール、学習用の本を確認する</span></a></div></section>
    ...
    <section className="services-list" id="broker"><div className="services-heading"><p className="eyebrow">BROKERAGE ACCOUNTS</p><h2>証券会社を比較する</h2><p>まず利用目的と取扱範囲を確認します。気になる会社だけ詳細ページで費用・注意点・向かない人まで確認してください。情報確認日：2026年9月15日。</p></div><div className="service-grid broker-summary-grid">{brokers.map((item) => { const detail=serviceDetails.find((service)=>service.name===item.name); return <article className="service-card broker-summary-card" key={item.name}><span>証券会社</span><h3>{item.name}</h3><p className="service-purpose-copy">{item.purpose}</p><ul>{item.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><div className="service-caution"><b>申込み前の確認</b><p>{item.caution}</p></div>{detail && <a className="service-detail-link" href={`/services/${detail.slug}`}>{item.name}の詳細を見る →</a>}{item.brokerDetails && <a className="official-source" href={item.brokerDetails.sourceUrl} target="_blank" rel="noopener noreferrer">公式情報を確認 ↗</a>}{item.affiliateCode && <AffiliateLink code={item.affiliateCode} />}</article>; })}</div></section>

    <section className="ideco-services" id="ideco"><div className="services-heading"><p className="eyebrow">iDeCo PROVIDER</p><h2>iDeCoの金融機関選び</h2><p>iDeCoは通常の証券口座と異なり、原則60歳まで自由に引き出せません。まず制度を理解し、利用すると決めた場合だけ商品・費用・支援体制を比べます。</p></div><div className="ideco-learning-link"><p>制度の違いを先に確認したい方へ</p><a href="/nisa-vs-ideco">NISAとiDeCoの違いを学ぶ →</a></div><div className="ideco-provider-grid"><article className="ideco-provider-card"><header><span>iDeCo</span><h3>松井証券 iDeCo</h3><p>老後資金づくりのためにiDeCoを利用すると決めた人が、金融機関を比較する際の候補です。</p></header><dl><div><dt>運営管理手数料等</dt><dd>松井証券の運営管理手数料は0円。制度上の手数料や商品の信託報酬等は別途かかります。</dd></div><div><dt>注意点</dt><dd>原則60歳まで自由に引き出せず、運用商品には元本割れの可能性があります。</dd></div></dl><a className="service-detail-link" href="/services/matsui-ideco">松井証券 iDeCoの詳細を見る →</a><div className="affiliate-link"><small>PR</small><span dangerouslySetInnerHTML={{ __html: matsuiIdecoAffiliateCode }} /></div></article><article className="ideco-provider-card"><header><span>iDeCo</span><h3>マネックス証券 iDeCo</h3><p>商品・費用・サポートを確認し、iDeCoの金融機関を比較する際の候補です。</p></header><dl><div><dt>運営管理手数料等</dt><dd>運営管理手数料は無料と案内されています。制度上の手数料や商品の信託報酬等は別途かかります。</dd></div><div><dt>注意点</dt><dd>原則60歳まで自由に引き出せず、運用商品には元本割れの可能性があります。</dd></div></dl><a className="service-detail-link" href="/services/monex-ideco">マネックス証券 iDeCoの詳細を見る →</a><a className="official-source" href="https://info.monex.co.jp/ideco/" target="_blank" rel="noopener noreferrer">公式参照先｜マネックス証券 iDeCo ↗</a></article></div></section>
    ...
    <section className="services-list resource-services" id="resources"><div className="services-heading"><p className="eyebrow">PRACTICE & LEARNING</p><h2>練習・分析・参考書</h2><p>実際のお金を使う前の練習、チャート確認、理解を深める参考資料です。口座比較とは分けて掲載します。</p></div><div className="service-grid resource-summary-grid">{resources.map((item) => <ResourceCard item={item} key={item.name} />)}</div></section>
  </main>;
}
