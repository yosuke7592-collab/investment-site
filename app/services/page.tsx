...
export default function ServicesPage() {
  ...
  return <main className="services-page">
    ...
    <section className="services-list"><div className="services-heading"><p className="eyebrow">COMPARE</p><h2>サービス・参考書</h2><p>掲載内容は2026年8月9日時点の公式情報を基にしています。申込前に必ず公式サイトで最新条件を確認してください。</p></div><div className="service-filters">{categories.map((item) => <button key={item.id} className={category === item.id ? "active" : ""} onClick={() => setCategory(item.id)}>{item.name}</button>)}</div><div className="service-grid">{visible.map((item) => <article className="service-card" key={item.name}><span>{categories.find((cat) => cat.id === item.category)?.name}</span><h3>{item.name}</h3><p className="service-purpose">{item.purpose}</p><ul>{item.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><div className="service-caution"><b>確認点</b><p>{item.caution}</p></div><a href={item.url} target="_blank" rel="noopener noreferrer">{item.label} <b>↗</b></a></article>)}</div></section>

    <section className="affiliate-ready"><p className="eyebrow">AFFILIATE READY</p><h2>将来リンクを入れる時も、<br/>教材を広告より先に置く。</h2><div><span>比較基準を固定</span><p>報酬額で順位を変えない</p><span>広告を明示</span><p>リンク付近に「広告」を表示</p><span>デメリットも掲載</span><p>向かない人と注意点を消さない</p><span>更新日を表示</span><p>料金・条件・提供状況を定期確認</p></div><nav><a href="/long-term">長期資産形成コース →</a><a href="/strategies">データ・戦略ラボ →</a></nav></section>
  </main>;
}
