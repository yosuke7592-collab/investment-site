...
type Service = { category: string; name: string; purpose: string; features: string[]; caution: string; url: string; label: string; affiliateCode?: string };
...
const services: Service[] = [
  ...
  { category: "broker", name: "DMM 株", purpose: "日本株・米国株を一つの口座で検討する候補", features: ["NISA", "日本株・米国株", "取引ツール・アプリ"], caution: "取引商品ごとの手数料・為替関連費用・注文方法を公式サイトで確認する。投資には元本割れなどのリスクがある。", url: "https://h.accesstrade.net/sp/cc?rk=0100mkk300oy0o", label: "DMM 株を見る", affiliateCode: dmmKabAffiliateCode },
...
export default function ServicesPage() {
  ...
  return <main className="services-page">
    ...
    <section className="services-list"><div className="services-heading"><p className="eyebrow">COMPARE</p><h2>サービス・参考書</h2><p>掲載内容は2026年9月3日時点の公式情報・広告主情報を基にしています。申込前に必ず公式サイトで最新条件を確認してください。</p></div><div className="service-filters">{categories.map((item) => <button key={item.id} className={category === item.id ? "active" : ""} onClick={() => setCategory(item.id)}>{item.name}</button>)}</div><div className="service-grid">{visible.map((item) => <article className="service-card" key={item.name}><span>{categories.find((cat) => cat.id === item.category)?.name}</span><h3>{item.name}</h3><p className="service-purpose">{item.purpose}</p><ul>{item.features.map((feature) => <li key={feature}>✓ {feature}</li>)}</ul><div className="service-caution"><b>確認点</b><p>{item.caution}</p></div>{item.affiliateCode ? <div className="affiliate-link"><small>PR</small><span dangerouslySetInnerHTML={{ __html: item.affiliateCode }} /></div> : <a href={item.url} target="_blank" rel="noopener noreferrer">{item.label} <b>↗</b></a>}</article>)}</div></section>
