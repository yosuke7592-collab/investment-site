...
export default function PatternsPage(){return <main className="patterns-page"><SiteHeader current="courses"/>
 <section className="deep-hero patterns-hero"><p className="eyebrow">CHART PATTERN ATLAS</p><h1>名前ではなく、<br/><em>値動きの理由を読む。</em></h1><p>ローソク足とチャートパターンは別物です。まず「基本」だけを学び、必要になったら「深掘り」へ進みます。</p><nav>{groups.map(g=><a href={`#${g.id}`} key={g.id}>{g.title.replace("パターン","")}</a>)}</nav></section>
 ...
 <section className="pattern-end"><p className="eyebrow">DO NOT COLLECT SHAPES</p><h2>使う形は、<br/>最初は3つでいい。</h2><p>一度に全部覚えません。自分の時間軸で見つけやすい形を3つ選び、同じ成立条件・損切り条件で記録します。</p><a href="/strategies/methods">手法と戦略を作る →</a></section>
