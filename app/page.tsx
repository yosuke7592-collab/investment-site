...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="top"><div className="hero-copy"><p className="eyebrow">20 LESSONS + TRADING LAB</p><h1>未来のお金を、<br/><em>自分で考える力</em>を。</h1><p className="lead">基礎を知るだけで終わらない。商品を比べ、取引画面を読み、注文と損益を練習してから、自分のルールで最初の少額投資へ進みます。</p><div className="hero-actions"><button className="primary" onClick={() => document.getElementById("curriculum")?.scrollIntoView({behavior:"smooth"})}>学習の地図を見る <span>→</span></button><a className="secondary" href="#choose-path">2つのコースを見る</a></div></div><div className="hero-orbit" aria-hidden="true"><div className="orbit"/><div className="orbit orbit-two"/><div className="coin"><span>¥</span></div><span className="orbit-label label-a">THINK</span><span className="orbit-label label-b">LEARN</span><span className="orbit-label label-c">DECIDE</span></div></section>
    <section className="course-intro"><p className="eyebrow">HOW TO LEARN</p><h2>一度で覚えなくていい。<br/>使いながら身につける。</h2><div className="intro-grid"><div><b>01</b><strong>基礎編</strong><p>お金・リスク・経済の土台を作る。</p></div><div><b>02</b><strong>取引準備編</strong><p>口座・商品・指標・手法を学ぶ。</p></div><div><b>03</b><strong>自分の道を選ぶ</strong><p>長期資産形成か、個別株・アクティブ投資へ進む。</p></div></div><div className="journey"><p className="eyebrow">FOUNDATION → PREPARATION → CHOOSE YOUR PATH</p><div><span>守る</span><i>→</i><span>基礎を知る</span><i>→</i><span>商品を比べる</span><i>→</i><span>道を選ぶ</span><i>→</i><span>ルールを作る</span><i>→</i><span>続ける</span></div><p>共通の基礎を学んだ後、目的に合わせて2つのコースへ分かれます。どちらも最後は、自分のルールを作り守ることにつながります。</p></div></section>
